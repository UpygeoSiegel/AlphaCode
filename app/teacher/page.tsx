...
import { groupTopics } from "@/lib/curriculumGrouping";
...
export default function TeacherDashboard() {
  ...
  // One accordion row per unit, labelled with its course, in curriculum order.
  const grouped = useMemo(
    () =>
      groupTopics(filtered).flatMap((course) =>
  ...
  useEffect(() => {
    if (search.trim()) {
      setExpandedCategories(new Set(grouped.map((g) => g.id)));
    }
  }, [search, grouped]);
    ...
    >
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {grouped.map(({ id: catId, label, courseName, topics: unitTopics }) => {
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3">
                    <div>
                      <span className="font-bold text-sm text-dm-navy">{label}</span>
                      <span className="ml-2 text-xs text-gray-400">{courseName}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <span className="text-xs font-semibold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">
                      {unitTopics.length} skill{unitTopics.length !== 1 ? "s" : ""}
                    </span>
                    <svg
